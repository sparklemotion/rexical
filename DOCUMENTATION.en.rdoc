...
== About
  ...
  Lexical Scanner Generator for Ruby and used with Racc.
...
=== state: Start State ( Optional )
    ...
    When uppercase letters follow the ":", the state becomes an exclusive start state.
    When lowercase letters follow, the state becomes an inclusive start state.
...
=== actions: Processing Actions ( Optional )
    ...
    The active defines the process for creating the appropriate token.
    A token is a two-element array of the type and the value, or is nil.
    ...
    Don't use functions that exit the block and change the control flow.
...
== Comment ( Optional )
  ...
  Any text following "#" to the end of the line becomes a comment.
...
=== scan_setup()
    ...
    This is the initialization event when the execution of the scanner starts.
...
== Notice
  ...
  This specification is provisional and may be changed without warning.
